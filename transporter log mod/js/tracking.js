...
const LogisticsTracking = {
  ...
  renderLiveMap: function(booking) {
    ...
    // Check if Google Maps is available and active
    if (this.useGoogleMaps && typeof google !== 'undefined' && typeof google.maps !== 'undefined') {
      this.initGoogleMap(container, booking, route);
  ...
  /**
   * Initialize Google Maps API instance with multi-stop Directions & custom truck radar
   */
  initGoogleMap: function(container, booking, route) {
    if (this.map) {
    ...
    try {
      ...
      // Truck marker
      this.googleTruckMarker = new google.maps.Marker({
        position: { lat: vehiclePos.lat, lng: vehiclePos.lng },
        map: this.googleMap,
        title: "Tata Ace EV (TN-33-AX-8910) · Karthik Raja",
        icon: {
          url: "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(`
            <svg xmlns="http://www.w3.org/2000/svg" width="46" height="46" viewBox="0 0 46 46">
              <circle cx="23" cy="23" r="20" fill="#0c5a36" stroke="#ffffff" stroke-width="3"/>
              <text x="23" y="28" font-size="18" text-anchor="middle" fill="#ffffff">🚚</text>
            </svg>
          `),
      ...
      route.forEach(pt => {
        if (pt.stopNum) {
          new google.maps.Marker({
            ...
            map: this.googleMap,
            title: `Stop ${pt.stopNum}: ${pt.farmer}`,
            label: { text: String(pt.stopNum), color: "#ffffff", fontWeight: "bold" },
            icon: {
      ...
      // Request route via Google Directions Service
      const directionsService = new google.maps.DirectionsService();
      this.googleDirectionsRenderer = new google.maps.DirectionsRenderer({
        map: this.googleMap,
        suppressMarkers: true,
        polylineOptions: { strokeColor: "#0c5a36", strokeWeight: 6, strokeOpacity: 0.85 }
      });

      directionsService.route({
        origin: new google.maps.LatLng(11.3410, 77.7172),
        destination: new google.maps.LatLng(11.0065, 76.9625),
        waypoints: [
          { location: new google.maps.LatLng(11.3168, 77.6912), stopover: true },
          { location: new google.maps.LatLng(11.2785, 77.5834), stopover: true },
          { location: new google.maps.LatLng(11.4485, 77.6835), stopover: true }
        ],
        ...
        if (status === google.maps.DirectionsStatus.OK) {
          ...
          // Coordinate Polyline Fallback if Directions API status is not OK
  ...
  recenterOnVehicle: function() {
    if (this.googleMap && this.googleTruckMarker) {
      this.googleMap.panTo(this.googleTruckMarker.getPosition());
  ...
  fitFullRoute: function() {
    if (this.googleMap && this.googleDirectionsRenderer && this.googleDirectionsRenderer.getDirections()) {
      this.googleMap.fitBounds(this.googleDirectionsRenderer.getDirections().routes[0].bounds);
      return;
    }
...
// Global Google Maps Load & Error Listeners
if (typeof window !== 'undefined') {
  window.onGoogleMapsApiLoaded = function() {
    console.log("Google Maps API loaded successfully.");
    if (typeof LogisticsTracking !== 'undefined' && LogisticsTracking.activeBooking) {
      LogisticsTracking.useGoogleMaps = true;
      const container = document.getElementById('live-map-canvas');
      if (container) {
        LogisticsTracking.renderLiveMap(LogisticsTracking.activeBooking);
      }
    }
  };

  window.gm_authFailure = function() {
    console.warn("Google Maps requires billing on Google Cloud. Maintaining high-reliability OpenStreetMap routing.");
    if (typeof LogisticsTracking !== 'undefined') {
      LogisticsTracking.useGoogleMaps = false;
      const container = document.getElementById('live-map-canvas');
      if (container && LogisticsTracking.activeBooking) {
        LogisticsTracking.initLeafletMap(container, LogisticsTracking.activeBooking, LogisticsTracking.currentRoute);
      }
    }
  };
}
